...
const TwentyOneDaysTestimonials = () => {
    ...
    return (
        <section className="bg-white py-8 px-4 overflow-hidden w-full">
            <div className="text-center mb-6 px-2">
                <h2 className="text-[24px] leading-[30px] font-semibold text-[#0D468B] mb-2">
                    Testimonials
                </h2>
                <h5 className="text-[18px] leading-[26px] font-semibold text-[#0D468B] mb-4">
                    Hear from our Healthyday Heroes
                </h5>
                <p className="text-[12px] leading-[22px] text-[#919191] italic">
                    The testimonials shown here are from real community members.{" "}
                    <span className="underline font-semibold">Individual results vary</span>{" "}
                    based on consistency, diet, lifestyle, and personal health factors.{" "}
                    <span className="underline font-semibold">Results are not typical or guaranteed.</span>{" "}
                    Yoga supports overall wellness but does not replace medical care. Please consult your
                    healthcare provider before starting any new program –{" "}
                    <strong>
                        <a
            ...
            {/* Single-card carousel — card width is 100% of this section */}
            <div
                ref={scrollRef}
                className="flex gap-6 overflow-x-auto snap-x snap-mandatory hide-scrollbar pb-4 mb-4 w-full px-4 scroll-px-4"
            >
                {testimonials.map((t, idx) => (
                    <div
                        key={idx}
                        className="bg-white rounded-[30px] shadow-[0px_0px_20px_0px_rgba(0,0,0,0.1)] p-6 flex flex-col items-center border border-slate-50 h-auto w-full shrink-0 snap-center"
                    >
            ...
            <div className="flex justify-center gap-2">
                {testimonials.map((_, idx) => (
                    <button
                        ...
                        className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${idx === 0 ? 'bg-[#ffb129]' : 'bg-[#ffeebb]'}`}
...
export default TwentyOneDaysTestimonials;
